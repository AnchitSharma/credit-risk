...
def class_pan_card(wd_list):
    also_check_app_word = ["income tax department", 'govt. of india', 'permanent account number']
    #
    vb_2 = [wd for wd in wd_list if
            (re.search(r"([a-zA-Z]{5}[1-9]{4}[a-zA-Z])", wd)) or any(x in wd.lower() for x in also_check_app_word)]
    if len(vb_2) > 0:
        return True
...
def class_loan_sanc(wd_list):
    also_check_app_word = ["loan sanction letter", 'sanctioned loan amount']
    vb_2 = [wd for wd in wd_list if any(x in wd.lower() for x in also_check_app_word)]
    if len(vb_2) > 0:
        return True
    return False
...
def class_bank_mandate(wd_list):
    also_check_app_word = ["mandate instruction form"]
    vb_2 = [wd for wd in wd_list if any(x in wd.lower() for x in also_check_app_word)]
    if len(vb_2) > 0:
        return True

    vb_1 = [wd for wd in wd_list if all(x in wd.lower() for x in ['authorizing', 'debit', 'account'])]
    if len(vb_1) > 0:
        return True
...
# MARGIN MONEY CSTMR TO DLR
# SHORT RECEIVED(INITIAL MONEY)
def class_disbur_memo(wd_list):
    vb_1 = [wd for wd in wd_list if all(x in wd.lower() for x in ['disbursement', 'memo'])]
    if len(vb_1) > 0:
        return True
    # Breakup of Total Upfront Charges
    vb_1 = [wd for wd in wd_list if all(x in wd.lower() for x in ['breakup', 'total', 'upfront', 'charges'])]
    if len(vb_1) > 0:
        return True
    # charge name rate amount calculated on charge amount
    vb_1 = [wd for wd in wd_list if
            all(x in wd.lower() for x in ['charge', 'name', 'rate', 'amount', 'calculated', 'charge'])]
    if len(vb_1) > 0:
        return True

    # MARGIN MONEY RECD FRM CUSTOMER
    vb_1 = [wd for wd in wd_list if
            all(x in wd.lower() for x in ['margin', 'money', 'recd', 'customer'])]
    if len(vb_1) > 0:
        return True
...
# Approval Cum Delivery Order
def class_delivery_order(wd_list):
    vb_1 = [wd for wd in wd_list if all(x in wd.lower() for x in ['approval', 'cum', 'delivery', 'order'])]
    if len(vb_1) > 0:
        return True
...
# wish to apply for following additional services/products through bajaj finance limited:
# We hereby apply for finance facility for purchase of product mentioned in this application form
def class_loan_app_back(wd_list):
    vb_1 = [wd for wd in wd_list if all(x in wd.lower() for x in ['wish', 'apply', 'additional'])]
    if len(vb_1) > 0:
        return True
    vb_1 = [wd for wd in wd_list if all(x in wd.lower() for x in ['services', 'products', 'bajaj', 'through'])]
    if len(vb_1) > 0:
        return True
    vb_1 = [wd for wd in wd_list if all(x in wd.lower() for x in ['apply', 'finance', 'facility', 'purchase'])]
    if len(vb_1) > 0:
        return True
    # Signature cars/AmExcard
    vb_1 = [wd for wd in wd_list if all(x in wd.lower() for x in ['signature', 'amexcard'])]
    if len(vb_1) > 0:
        return True
...
def class_CAM_List(wd_list):
    # CAM Listing
    vb_1 = [wd for wd in wd_list if all(x in wd.lower() for x in ['cam', 'apply'])]
    if len(vb_1) > 0:
        return True
    # Single/ Multiple Vehicles
    vb_1 = [wd for wd in wd_list if all(x in wd.lower() for x in ['single', 'multiple', 'vehicles'])]
    if len(vb_1) > 0:
        return True
...
# SCHEDULE FORMING PART OF AUTO LOAN AGREEMENT
def class_schedule_form(wd_list):
    # CAM Listing
    vb_1 = [wd for wd in wd_list if all(x in wd.lower() for x in ['schedule', 'forming', 'agreement'])]
    if len(vb_1) > 0:
        return True
    # Agreement No. / Unique ID No.
    vb_1 = [wd for wd in wd_list if all(x in wd.lower() for x in ['agreement', 'unique', 'no'])]
    if len(vb_1) > 0:
        return True
    vb_1 = [wd for wd in wd_list if all(x in wd.lower() for x in ['place', 'of', 'agreement'])]
    if len(vb_1) > 0:
        return True
...
def text_classify(wd_list, doc_path):
    if class_loan_app(wd_list):
        db_model.create_kyc_doc(conn, (
            doc_path, "LOAN APPLICATION FRONT", '\n'.join(wd_list), None, None, None, None, None, None))
        return
    elif class_loan_app_back(wd_list):
        db_model.create_kyc_doc(conn,
                                (doc_path, "LOAN APPLICATION BACK", '\n'.join(wd_list), None, None, None, None, None,
                                 None))
        return
    elif class_disbur_memo(wd_list):
        db_model.create_kyc_doc(conn, (doc_path, "Disbursement Memo", '\n'.join(wd_list), None, None, None, None, None,
                                       None))
        return
    elif class_CAM_List(wd_list):
        db_model.create_kyc_doc(conn, (doc_path, "CAM Listing", '\n'.join(wd_list), None, None, None, None, None,
                                       None))
        return
    elif class_bank_mandate(wd_list):
        db_model.create_kyc_doc(conn, (doc_path, "Bank Mandate Form", '\n'.join(wd_list), None, None, None, None, None,
                                       None))
        return
    elif class_delivery_order(wd_list):
        db_model.create_kyc_doc(conn, (doc_path, "Delivery Order", '\n'.join(wd_list), None, None, None, None, None,
                                       None))
        return
    elif class_insur_doc(wd_list):
        db_model.create_kyc_doc(conn, (doc_path, "Insurance Document", '\n'.join(wd_list), None, None, None, None, None,
                                       None))
        return
    elif class_loan_sanc(wd_list):
        db_model.create_kyc_doc(conn,
                                (doc_path, "Loan Sanction Document", '\n'.join(wd_list), None, None, None, None, None,
                                 None))
        return
    elif class_schedule_form(wd_list):
        db_model.create_kyc_doc(conn, (doc_path, "Schedule Form", '\n'.join(wd_list), None, None, None, None, None,
                                       None))
    ...
    if op:
        ...
        m.doc_type = "Aadhaar+Pan" if is_pan else "Aadhaar"
...
if __name__ == '__main__':
    ...
    for mdl in kycdoc_list:
        if (mdl is not None) and ('Aadhaar' in mdl.doc_type):
            ...
            if conn:
                task = (m.doc_name, m.doc_type, '\n'.join(m.extract_text),
                        str(m.is_adhaar_type), m.adhaar_num, m.is_adhaar_name,
                        m.is_adhaar_dob, m.is_adhaar_gender, m.is_adhaar_address)
                db_model.create_kyc_doc(conn, task)
